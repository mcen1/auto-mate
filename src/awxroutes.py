...
def removeTypicalAWXGarbage(tosay):
  ...
  regex_replacement="<br>"
  regex_search_term='''
https://iambyti1p002.company.com/BeyondTrust/api/public/v3\n/usr/local/lib/.*/site-packages/urllib3/connectionpool.py:.*: InsecureRequestWarning: Unverified HTTPS request is being made to host .*\. Adding certificate verification is strongly advised. See:.*\n.*warnings.warn\('''
  tosay=re.sub(regex_search_term, regex_replacement, tosay)
  return tosay

# example  http://localhost:5000/runjobawxreqconf?awx-job-name=automation-awx-jobtesting-job&extravarfavoritecolor=aaa&extravarfavoriteanimal=bbbb
# run job via URL but with confirmation dialog
...
def post_url_to_awx2():
  ...
  if sessioncheckresults!="ok":
    return render_template('login.html', redirecturl=f"{URLPREFACE}/runjobawxreqconf?{assjobparams}",URLPREFACE=URLPREFACE,userinfo=f"Please log in with your ABC Active Directory account to continue to runjobawxreqconf?{assjobparams}.")
  try:
  ...
  if canrun==False:
    e=f"{username} is not in a group that is allowed to run this job"
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  else:
    print("attempting render template")
    expiry=renewDBSession(sessionid)
    resp=make_response(render_template('runjobconfirm.html',reqargs=request.args.to_dict(),URLPREFACE=URLPREFACE))
    resp.set_cookie('sessionid', sessionid, httponly=True, secure=cookiesecurity, samesite='Lax',expires=expiry)
    return resp
    #return render_template('runjobconfirm.html',reqargs=request.args.to_dict(),URLPREFACE=URLPREFACE)
...
def post_url_to_awx():
  ...
  if canrun==False:
    ...
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  print(f"Attempting to run via middleware {username} is username and request form is {request.form} ")
  jobID=runITOAPIJob(request.args,username,{})
  jobsan=jobID
  try:
    jobsan=json.loads(jobID)
    if "errors" in jobsan:
      if "credentials" in jobsan["errors"]:
        print(f"Real output hidden from user. Job output was {jobID}")
        jobsan["errors"]["credentials"]="Error setting credentials for job run."
  except Exception as e:
    print(f"Exception putting output to json: {e}")
  if "results" in jobID:
    ...
    print("attempting render template")
    return render_template('errorrunningjob.html',jobdata=f"There was an error launching the job. Received this reply when trying to launch: {jobsan}",URLPREFACE=URLPREFACE)
...
def post_to_awx():
  ...
  if canrun == False:
    ...
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  jobID=runITOAPIJob(request.form,username,{})
  jobsan=jobID
  try:
    jobsan=json.loads(jobID)
    if "errors" in jobsan:
...
def cancel_job_awx():
  ...
  try:
    jobname=getJobName(request.form["jobid"])
  except Exception as e:
    return {"error": f"There was an error launching the job. Received this reply when trying to launch: {e}"}
  username=retrieveDBData(sessionid,"username")
  ...
  if canrun == False:
    ...
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  jobData=cancelJobITOAPI({"job_id": request.form["jobid"]})
  jobresults=json.loads(jobData)
  tosay=jobresults
...
def get_awx_job_status():
  ...
  if canrun == False:
    ...
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  jobData=getJobStatus(request.args["jobid"])
  return jobData.replace("successful","finished")
...
def follow_on_awx_ajax():
  ...
  if canrun == False:
    ...
    errorcode=403
    return render_template('error.html',data=e,errorcode=errorcode,URLPREFACE=URLPREFACE)
  jobData=getOutputITOAPI(request.args["jobid"],"html")
  ...
  tosay=jobresults.replace('&lt;br&gt;','<br>')
  return tosay
